...
int socfpga_bridge_nongraceful_disable(uint32_t f2s_force_drain, uint32_t f2s_respempty)
{
	int ret = 0;
	int timeout = 300;
...
/**************** SOC2FPGA ****************/
	...
	if ((brg_mask & RSTMGR_BRGMODRSTMASK_SOC2FPGA) != 0) {
		...
		/* Clear System Manager soc bridge control register
		[soc2fpga_ready_latency_enable] = 1 */
...
/**************** FPGA2SDRAM ****************/
	...
	if (brg_mask != 0U) {
		...
		ret = socfpga_bridge_nongraceful_disable(f2s_force_drain, f2s_respempty);
