		...
		uint32_t dst_addr, uint32_t dst_size, uint32_t *send_id)
{
	...
	inv_dcache_range(src_addr, src_size);
				...
				uint32_t param_size, uint32_t *mbox_error)
{
	...
	/*
	 * Check if not ECB, CBC and CTR, GCM and GCM-GHASH mode (only for Agilex3),
				...
				uint64_t smmu_dst_addr)
{
	 ...
	 * On the Agilex3 platform with GCM and GCM-GHASH modes, the source and destination size
