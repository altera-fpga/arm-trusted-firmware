...
int cad_qspi_n25q_wait_for_program_and_erase(int program_only)
{
	...
	while (count < CAD_QSPI_COMMAND_TIMEOUT) {
		...
		if (((program_only &&
			CAD_QSPI_STIG_FLAGSR_PROGRAMREADY(flag_sr)) != 0) ||
			((!program_only &&
			CAD_QSPI_STIG_FLAGSR_ERASEREADY(flag_sr)) != 0))
			break;
	}
	...
	if (((program_only && CAD_QSPI_STIG_FLAGSR_PROGRAMERROR(flag_sr)) != 0) ||
			((!program_only &&
			CAD_QSPI_STIG_FLAGSR_ERASEERROR(flag_sr)) != 0)) {
