...
void bl31_plat_arch_setup(void)
{
	...
	boot_core = ((mmio_read_32(AGX5_PWRMGR(MPU_BOOTCONFIG)) & 0xC00) >> MPU_BOOTCONFIG_VAL);
...
uint32_t arm_get_spsr_for_bl33_entry(void)
{
	...
	mode = (el_implemented(MPU_MODE_EL2) != EL_IMPL_NONE) ? MODE_EL2 : MODE_EL1;
...
void bl31_plat_set_secondary_cpu_entrypoint(unsigned int cpu_id)
{
	...
	boot_core = (mmio_read_32(AGX5_PWRMGR(MPU_BOOTCONFIG)) & MPU_BOOTCONFIG_MASK);
...
(uint64_t) plat_secondary_cpus_bl31_entry >> CPU_RESETBASELOW_CPU_SHIFT);
	} else {
		/* Update reset vector to 0x00 */
...
(uint64_t) plat_secondary_cpus_bl31_entry >> CPU_RESETBASELOW_CPU_SHIFT);
	...
	/* Update reset vector to 0x00 */
	mmio_write_64(RSTMGR_CPUxRESETBASELOW_CPU1, (uint64_t) plat_secondary_cpus_bl31_entry >> CPU_RESETBASELOW_CPU_SHIFT);
	mmio_write_64(RSTMGR_CPUxRESETBASELOW_CPU3, (uint64_t) plat_secondary_cpus_bl31_entry >> CPU_RESETBASELOW_CPU_SHIFT);
...
void bl31_plat_set_secondary_cpu_off(void)
{
	...
	pch_cpu = pch_cpu & ~(pch_cpu_off << CPU_PCH_OFF_SHIFT);
