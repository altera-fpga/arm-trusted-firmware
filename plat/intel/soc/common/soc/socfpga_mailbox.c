				    ...
				    bool *is_doorbell_triggered)
{
	...
	do {
		if (is_mailbox_cmdbuf_full(*cin)) {
			/*
			 * Intermediate doorbell: Buffer is full. Trigger HW CIN update
			 * and early doorbell to notify SDM, then wait for buffer space.
					...
					unsigned int len)
{
	 ...
	 * Update CIN atomically before doorbell to ensure SDM reads complete data.
	 */
